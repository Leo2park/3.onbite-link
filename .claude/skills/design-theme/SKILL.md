...
- **라이트모드와 다크모드를 모두 구현한다** — 위의 "컬러 (라이트모드)"를 기본값으로 CSS 변수에 정의하고, `@media (prefers-color-scheme: dark)` 안에서 "컬러 (다크모드)" 값으로 오버라이드한다. 별도의 다크모드 토글 UI/`.dark` 클래스는 요청이 없는 한 추가하지 않고, 시스템 설정(`prefers-color-scheme`)만 따른다.
